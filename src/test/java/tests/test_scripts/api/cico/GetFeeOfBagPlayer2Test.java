...
import static common.utilities.Constants.GET_FEE_OF_BAG_PLAYER_ENDPOINT;
import static io.restassured.RestAssured.given;

public class GetFeeOfBagPlayer2Test extends TestConfig implements FlowRunnable {
    // ==== ĐƯỜNG DẪN — chỉnh cho khớp project của bạn ====
    private static final String EXCEL_FILE = System.getProperty("user.dir")
            + "/src/main/resources/input_excel_file/booking/CICO.xlsx";
    private static final String SHEET_NAME = "Fee_Of_Bag_Player2";
    // Thư mục chứa JSON request/expect cho API này
    private static final String JSON_DIR = System.getProperty("user.dir")
            + "/src/main/resources/input_json_file/cico/fee_of_bag/";

    // ======================= DataProvider =======================
    @DataProvider(name = "getFeeOfBagData")
    public Object[][] getFeeOfBagData() throws IOException {
     ...
     */
    @Test(dataProvider = "getFeeOfBagData")
    public void testGetFeeOfBag(Map<String, String> row, ITestContext ctx) throws IOException {
...
// Lấy từ context
        ...
        Response resp = given()
                ...
                .queryParams(q)
                .when()
                .get(BASE_URL + GET_FEE_OF_BAG_PLAYER_ENDPOINT)
                .then()
                .extract()
                .response();
        ...
        String url = BASE_URL + GET_FEE_OF_BAG_PLAYER_ENDPOINT;
